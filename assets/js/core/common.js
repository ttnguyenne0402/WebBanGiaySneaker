...
const bieuMauThanhToan = document.querySelector("#bieu-mau-thanh-toan");
const thongBaoThanhToan = document.querySelector("#thong-bao-thanh-toan");
const noiDungTomTat = document.querySelector("#noi-dung-tom-tat");
const tenNguoiNhan = document.querySelector("#ten-nguoi-nhan");
const dienThoaiNguoiNhan = document.querySelector("#dien-thoai-nguoi-nhan");
...
function loadProductData() {
  ...
  adminProducts.forEach((product) => {
    ...
    if (!key || deletedProductIds.has(key)) {
      return;
    }
...
function capNhatTieuDeDanhSach() {
  ...
  if (danhMucDaChon.length === 1 && thuongHieuDaChon.length === 0) {
    tieuDeDanhSachSanPham.textContent = danhMucDaChon[0];
    return;
  }

  if (danhMucDaChon.length > 0 || thuongHieuDaChon.length > 0) {
...
function moBoLoc() {
  if (!bangBoLoc || !nenBoLoc || !nutMoBoLoc) {
    return;
  }
...
function locSanPhamTheoGia(sanPham, khoangGia) {
  if (khoangGia === "duoi-2-trieu") {
    return sanPham.gia < 2000000;
  }

  if (khoangGia === "tu-2-den-3-trieu") {
    return sanPham.gia >= 2000000 && sanPham.gia <= 3000000;
  }

  if (khoangGia === "tren-3-trieu") {
    return sanPham.gia > 3000000;
  }
...
function laySanPhamHienThi() {
  ...
  if (sapXepSanPham?.value === "gia-giam") {
    sanPhamHienThi = sanPhamHienThi.sort((a, b) => b.gia - a.gia);
  }

  if (sapXepSanPham?.value === "giam-gia") {
...
function veGioHang() {
  ...
  const danhSachGio = document.querySelector("#danh-sach-gio-hang");
  const tongTienText = document.querySelector("#tong-tien-gio-hang");

  // 2. Cập nhật số lượng trên header (chạy được trên mọi trang)
  if (soLuongBadge) {
    const tongSoLuong = gioHang.reduce((tong, monHang) => tong + monHang.soLuong, 0);
    soLuongBadge.textContent = tongSoLuong;
  }

  // 3. Cập nhật tổng tiền (nếu ở trang giỏ hàng)
  if (tongTienText) {
    tongTienText.textContent = dinhDangTien(tinhTamTinh());
  }

  // 4. Nếu không phải trang giỏ hàng (không có danh sách), dừng lại
  if (!danhSachGio) return;

  // 5. Render danh sách sản phẩm
  if (gioHang.length === 0) {
    danhSachGio.innerHTML = '<div class="gio-hang-trong p-3 text-center text-muted">Giỏ hàng đang trống.</div>';
    return;
  }

  danhSachGio.innerHTML = gioHang.map((monHang) => {
    // Tạo link chi tiết
...
function moChiTietSanPham(maSanPham) {
  ...
  if (thuongHieuChiTiet) {
    thuongHieuChiTiet.textContent = sanPham.thuongHieu;
  }

  if (danhMucChiTiet) {
    danhMucChiTiet.textContent = sanPham.danhMuc;
  }

  if (doiTuongChiTiet) {
    doiTuongChiTiet.textContent = sanPham.doiTuong;
  }

  if (loaiChiTiet) {
    loaiChiTiet.textContent = sanPham.loai;
  }

  if (tonKhoChiTiet) {
    tonKhoChiTiet.textContent = sanPham.tonKho > 0 ? `${sanPham.tonKho} sản phẩm còn hàng` : "Tạm hết hàng";
  }

  if (breadcrumbName) {
    breadcrumbName.textContent = sanPham.ten;
  }
...
function getCheckoutAddressKey() {
  return nguoiDungHienTai?.ma ? `tht_user_addresses_${nguoiDungHienTai.ma}` : "tht_user_addresses";
}

function getCheckoutAddresses() {
  try {
    return JSON.parse(localStorage.getItem(getCheckoutAddressKey())) || [];
  } catch (error) {
    return [];
  }
}
...
function getSelectedCheckoutAddress() {
  ...
  const addresses = getCheckoutAddresses();
  const selectedId = select.value;
  return addresses.find((addr) => String(addr.id) === selectedId) || null;
}

function applySelectedCheckoutAddress() {
  const checkoutAddressGroup = document.getElementById("checkoutAddressGroup");
  const selectedAddress = getSelectedCheckoutAddress();
  const addressFields = [
    { id: "ten-nguoi-nhan", value: selectedAddress?.name || nguoiDungHienTai?.hoTen || "" },
    { id: "dien-thoai-nguoi-nhan", value: selectedAddress?.phone || nguoiDungHienTai?.dienThoai || "" },
    { id: "dia-chi-giao-hang", value: selectedAddress?.detail || "" },
    { id: "tinh-thanh", value: selectedAddress?.region || "" }
  ];

  addressFields.forEach(({ id, value }) => {
    const field = document.getElementById(id);
    if (!field) {
      return;
    }
    field.value = value;
    field.readOnly = !!selectedAddress;
  });

  if (checkoutAddressGroup) {
    checkoutAddressGroup.style.display = selectedAddress ? "block" : "block";
  }
}
...
function renderCheckoutAddressOptions() {
  ...
  select.innerHTML = addresses
    .map(
      (addr) => `
        <option value="${addr.id}">${addr.name} — ${addr.region} — ${addr.detail}</option>
      `
    )
    .join("");

  select.addEventListener("change", applySelectedCheckoutAddress);
  applySelectedCheckoutAddress();
}

function dienThongTinThanhToan() {
  if (!bieuMauThanhToan) {
    return;
  }

  bieuMauThanhToan.classList.remove("was-validated");
  xoaThongBaoBieuMau(thongBaoThanhToan);

  if (nguoiDungHienTai) {
    if (tenNguoiNhan) {
      tenNguoiNhan.value = nguoiDungHienTai.hoTen || "";
    }

    if (dienThoaiNguoiNhan) {
      dienThoaiNguoiNhan.value = nguoiDungHienTai.dienThoai || "";
    }
  }
...
function veTomTatDonHang() {
  ...
  if (gioHang.length === 0) {
    noiDungTomTat.innerHTML = '<div class="gio-hang-trong">Giỏ hàng đang trống.</div>';
    return;
  }

  const tamTinh = tinhTamTinh();
  const phiVanChuyen = tinhPhiVanChuyen();
  const danhSachTomTat = gioHang.map((monHang) => `
    <div class="san-pham-tom-tat">
      <img src="${monHang.anh}" alt="${monHang.ten}">
      <div>
        <h4>${monHang.ten}</h4>
        <p>Size ${monHang.kichThuoc} - ${monHang.mauSac}</p>
        <p>${monHang.soLuong} x ${dinhDangTien(monHang.gia)}</p>
        <button type="button" class="btn btn-sm btn-outline-danger xoa-gio mt-2" data-ma-dong="${monHang.maDong}">Xóa</button>
      </div>
    </div>
  `).join("");

  noiDungTomTat.innerHTML = `
    ${danhSachTomTat}
    <div class="dong-tom-tat">
      <span>Tạm tính</span>
      <span>${dinhDangTien(tamTinh)}</span>
    </div>
    <div class="dong-tom-tat">
      <span>Vận chuyển</span>
      <span>${phiVanChuyen === 0 ? "Miễn phí" : dinhDangTien(phiVanChuyen)}</span>
    </div>
    <div class="tong-tom-tat">
      <span>Tổng thanh toán</span>
      <span>${dinhDangTien(tinhTongThanhToan())}</span>
    </div>
  `;
}
...
function batDauThanhToan() {
  ...
  if (trangHienTai !== "checkout") {
    window.location.href = "checkout.html";
    return;
  }

  dienThongTinThanhToan();
  veTomTatDonHang();
  dongGioHang();
}
...
function thanhToanDonHang(suKien) {
  suKien.preventDefault();
  xoaThongBaoBieuMau(thongBaoThanhToan);
  bieuMauThanhToan.classList.add("was-validated");
  ...
  if (gioHang.length === 0) {
    hienThongBaoBieuMau(thongBaoThanhToan, "loi", "Giỏ hàng đang trống.");
    return;
  }

  if (!bieuMauThanhToan.checkValidity()) {
    hienThongBaoBieuMau(thongBaoThanhToan, "loi", "Vui lòng nhập đủ thông tin giao hàng hợp lệ.");
    return;
  }

  const maDonHang = `SS${Date.now().toString().slice(-8)}`;
  const tamTinh = tinhTamTinh();
  const phiVanChuyen = tinhPhiVanChuyen();
  const cachThanhToan = bieuMauThanhToan.querySelector('input[name="cachThanhToan"]:checked').value;
  const selectedAddress = getSelectedCheckoutAddress();
  const donHangMoi = {
    ma: maDonHang,
    maNguoiDung: nguoiDungHienTai.ma,
    khachHang: selectedAddress
      ? {
        hoTen: selectedAddress.name || nguoiDungHienTai.hoTen || "",
        dienThoai: selectedAddress.phone || nguoiDungHienTai.dienThoai || "",
        diaChi: selectedAddress.detail || "",
        tinhThanh: selectedAddress.region || "",
        ghiChu: document.querySelector("#ghi-chu-don-hang").value.trim()
      }
      : {
        hoTen: tenNguoiNhan.value.trim(),
        dienThoai: dienThoaiNguoiNhan.value.trim(),
        diaChi: document.querySelector("#dia-chi-giao-hang").value.trim(),
        tinhThanh: document.querySelector("#tinh-thanh").value.trim(),
        ghiChu: document.querySelector("#ghi-chu-don-hang").value.trim()
      },
    cachThanhToan,
    sanPham: gioHang.map((monHang) => ({ ...monHang })),
    tamTinh,
    phiVanChuyen,
  ...
  donHang.unshift(donHangMoi);
  ghiLuuTru(khoaLuuTru.donHang, donHang);

  gioHang = [];
  luuGioHang();
  veGioHang();
  veTomTatDonHang();
  bieuMauThanhToan.reset();
  bieuMauThanhToan.classList.remove("was-validated");
  hienThongBaoBieuMau(thongBaoThanhToan, "thanh-cong", `Thanh toán thành công. Mã đơn hàng: ${maDonHang}.`);
  hienThongBaoNhanh("Thanh toán thành công.");
}
...
danhSachGioHang?.addEventListener("click", (suKien) => {
  ...
  if (nut.classList.contains("tang-gio")) {
    capNhatSoLuongGioHang(maDong, "tang");
  }

  if (nut.classList.contains("giam-gio")) {
    capNhatSoLuongGioHang(maDong, "giam");
  }
...
document.addEventListener("keydown", (suKien) => {
  if (suKien.key === "Escape" && bangBoLoc?.classList.contains("dang-mo")) {
    dongBoLoc();
  }
});
...
function khoiTaoTrangChiTietSanPham() {
  if (trangHienTai !== "product-detail") {
    return;
  }
...
function veTrangThaiNguoiDung() {
  ...
  if (!hanhDongKhach || !hanhDongNguoiDung || !tenNguoiDung || !emailNguoiDung) {
    return;
  }
...
function initChatBox() {
  ...
  // Mở/đóng khi click vào nút Messenger
  ...
  // Tùy chọn: đóng chat khi click ra ngoài khung
